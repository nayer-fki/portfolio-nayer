  ...
  return (
    <main className="page-shell">
      ...
      <section className="mx-auto max-w-5xl px-6 py-14 sm:py-20">
        <div className="relative space-y-6 before:absolute before:bottom-5 before:left-5 before:top-5 before:hidden before:w-px before:bg-white/10 sm:before:left-[11.25rem] sm:before:block">
          {experiences.map((experience) => (
            <article key={`${experience.company}-${experience.role}`} className="relative grid gap-3 sm:grid-cols-[9rem_1fr] sm:gap-5">
              <div className="text-sm text-slate-500 sm:pt-6">
                <p>{experience.period}</p>
                <p className="mt-1 text-xs">{experience.location}</p>
              </div>
              <span className="absolute left-[10.9rem] top-8 hidden h-3 w-3 rounded-full border-2 border-[#070b14] bg-blue-400 ring-4 ring-blue-400/10 sm:block" />
              <div className="surface-card rounded-2xl p-5 sm:ml-8 sm:p-6">
